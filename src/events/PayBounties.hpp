...
//
//     pay_bounties data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, pay_bounties & x);
    void to_json(json & j, const pay_bounties & x);

    inline void from_json(const json & j, pay_bounties& x) {
    ...
    inline void to_json(json & j, const pay_bounties & x) {
        j = json::object();
