...
//
//     buy_weapon data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, buy_weapon & x);
    void to_json(json & j, const buy_weapon & x);

    inline void from_json(const json & j, buy_weapon& x) {
        x.buy_weapon_class = j.at("Class").get<int64_t>();
    ...
    inline void to_json(json & j, const buy_weapon & x) {
        j = json::object();
        j["Class"] = x.buy_weapon_class;
        j["event"] = x.event;
        j["Name"] = x.name;
