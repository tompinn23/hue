...
//
//     dock_srv data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, dock_srv & x);
    void to_json(json & j, const dock_srv & x);

    inline void from_json(const json & j, dock_srv& x) {
    ...
    inline void to_json(json & j, const dock_srv & x) {
        j = json::object();
        j["event"] = x.event;
        j["ID"] = x.id;
