...
//
//     book_taxi data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, book_taxi & x);
    void to_json(json & j, const book_taxi & x);

    inline void from_json(const json & j, book_taxi& x) {
    ...
    inline void to_json(json & j, const book_taxi & x) {
        j = json::object();
        j["Cost"] = x.cost;
