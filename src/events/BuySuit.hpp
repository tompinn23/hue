...
//
//     buy_suit data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, buy_suit & x);
    void to_json(json & j, const buy_suit & x);

    inline void from_json(const json & j, buy_suit& x) {
    ...
    inline void to_json(json & j, const buy_suit & x) {
        j = json::object();
        j["event"] = x.event;
        j["Name"] = x.name;
