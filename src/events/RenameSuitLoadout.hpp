...
//
//     rename_suit_loadout data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, rename_suit_loadout & x);
    void to_json(json & j, const rename_suit_loadout & x);

    inline void from_json(const json & j, rename_suit_loadout& x) {
    ...
    inline void to_json(json & j, const rename_suit_loadout & x) {
        j = json::object();
        j["event"] = x.event;
