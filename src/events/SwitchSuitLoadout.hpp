...
//
//     switch_suit_loadout data = nlohmann::json::parse(jsonString);
...
namespace hue {
    ...
    /**
     * This event is logged when a player selects a different flight suit from the shipΓÇÖs locker
     */
...
namespace hue {
    ...
    void from_json(const json & j, switch_suit_loadout & x);
    void to_json(json & j, const switch_suit_loadout & x);


    inline void from_json(const json & j, switch_suit_loadout& x) {
    ...
    inline void to_json(json & j, const switch_suit_loadout & x) {
        j = json::object();
        j["event"] = x.event;
