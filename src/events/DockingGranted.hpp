...
//
//     docking_granted data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, docking_granted & x);
    void to_json(json & j, const docking_granted & x);

    inline void from_json(const json & j, docking_granted& x) {
        x.event = j.at("event").get<std::string>();
        x.landing_pad = j.at("LandingPad").get<int64_t>();
    ...
    inline void to_json(json & j, const docking_granted & x) {
        j = json::object();
        j["event"] = x.event;
        j["LandingPad"] = x.landing_pad;
