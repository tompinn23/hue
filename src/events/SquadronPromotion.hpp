...
//
//     squadron_promotion data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, squadron_promotion & x);
    void to_json(json & j, const squadron_promotion & x);

    inline void from_json(const json & j, squadron_promotion& x) {
    ...
    inline void to_json(json & j, const squadron_promotion & x) {
        j = json::object();
        j["event"] = x.event;
