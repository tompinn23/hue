...
//
//     touchdown data = nlohmann::json::parse(jsonString);
...
namespace hue {
    void from_json(const json & j, touchdown & x);
    void to_json(json & j, const touchdown & x);

    inline void from_json(const json & j, touchdown& x) {
    ...
    inline void to_json(json & j, const touchdown & x) {
        j = json::object();
        j["Body"] = x.body;
        j["BodyID"] = x.body_id;
        j["event"] = x.event;
